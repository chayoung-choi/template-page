...
본 문서는 AI가 코드를 생성하고 구조를 설계할 때 반드시 준수해야 하는 절대적인 원칙(Rules)과 구조 명세서입니다. 모든 산출물은 아래의 가이드라인을 100% 충족해야
합니다.
...
* **resource:** 페이지의 리소스는 기본적으로 /image 경로에 있는 이미지들을 활용한다.
* **대시보드 갱신:** 새 페이지가 생성될 때마다 루트(Root) 경로에 있는 `index.html` (대시보드)에 해당 페이지로 이동할 수 있는 링크를 반드시 추가하여 인덱싱
  상태를 유지합니다.
* **Export 우클릭:** 새 페이지에도 항상 elementor-exporter 삽입한다.
* **예상 디렉토리 트리:**
  ```text
  / (root)
...
## 2. Elementor Export 함수 규칙

* **우클릭 Export 기능:** 선택된 섹션단위의 HTML 코드를 WordPress Elementor 요소들로 변환하여 Export json 파일로 생성해준다.
* WordPress, Elementor, Elementor Pro 블록 요소 단위로만 제공
* HTML 코드 요소 사용 금지(단, 위 제공 요소로 제공할 수 없는 경우 HTML 코드 블록 사용 가능)
...
* HTML에 선언된 stylesheet style.css는 변환된 블록의 custom_css 요소로 추가한다.

## 99. 답변 원칙

* 모든 답변의 마지막 요약/결론은 가능한 한 간략하게 작성하여 토큰 사용량을 최소화합니다.
